...
.header-btn{
  height:28px;min-height:28px;max-height:28px;padding:0 12px;
  border-radius:6px;border:1px solid #30363d;background:#21262d;
  color:#c9d1d9;font-size:12px;cursor:pointer;text-decoration:none;
...
.bcard{
  ...
  padding:14px;transition:.15s;cursor:default;
}
...
.bcard .bip{
  ...
  border-radius:4px;padding:3px 7px;display:inline-block;margin-bottom:10px;
}
...
.conn-btn{
  ...
  text-decoration:none;cursor:pointer;transition:.15s;border:1px solid;
}
...
function buildTabs(){
  ...
  for(const a of Object.keys(counts)){
    if(!areas.includes(a) && a!=='__ALL__') areas.push(a);
  }
  ...
  for(const a of areas){
    if(a==='__ALL__') continue;
    tabs.innerHTML += tabHTML(a, AREA_LABEL[a]||a, counts[a]||0);
  }
...
function render(){
  ...
  if(list.length===0){
    grid.innerHTML='<div class="empty" style="grid-column:1/-1">無符合資料</div>';
    return;
  }
  grid.innerHTML = list.map(b=>{
    const ip = b.ip||'';
    const base = ip ? `http://${ip}:9000` : '';
    const areaBadge = (currentArea==='__ALL__' && b.area) ? `<span class="area-badge">${b.area}</span>` : '';
    const btn = (label, hash, cls) => base
      ? `<a class="conn-btn ${cls}" href="${base}/webfig/#${hash}" target="_blank" rel="noopener">${label}</a>`
  ...
  <div class="btn-row">
    ${btn('📡 DHCP','dhcp-server/leases','blue')}
  </div>
</div>`;
  }).join('');
}
